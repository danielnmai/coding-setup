...
log stream --predicate 'composedMessage contains "AVFCapture"' | while read line; do
  ...
  if echo "$line" | grep -qE "AVCaptureSession_Tundra startRunning"; then
	...
	curl --location --request PUT 'http://192.168.40.244:9123/elgato/lights' --header 'Content-Type: application/json' --data-raw '{"lights":[{"brightness":40,"temperature":162,"on":1}],"numberOfLights":1}'
  ...
  if echo "$line" | grep -qE "AVCaptureSession_Tundra stopRunning"; then
	...
	#Light 1 - set to off
	curl --location --request PUT 'http://192.168.40.244:9123/elgato/lights' --header 'Content-Type: application/json' --data-raw '{"lights":[{"brightness":40,"temperature":162,"on":0}],"numberOfLights":2}'
